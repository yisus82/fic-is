...
public abstract class PreProcessingFilter {
         ...
         * call <code>process</code> on the next filter.
         */
        public ActionForward process(HttpServletRequest request,
                ...
                /* Process this filter. */
                try {
                        actionForward = doProcess(request, response, action,
                                        form, mapping);
                } catch (InternalErrorException e) {
                        return doOnInternalErrorException(request, response,
                                        action, form, mapping, e);
                }

                /* Process next filter in the chain. */
                if ((actionForward == null) && (nextFilter != null)) {
                        return nextFilter.process(request, response, action,
                                        form, mapping);
                }
        ...
        protected ActionForward doOnInternalErrorException(
                 ...
                 * error.
                 */
                ServletContext servletContext = action.getServlet()
